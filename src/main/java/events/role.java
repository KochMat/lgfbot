...
public class role extends ListenerAdapter {
    public void onGuildMemberJoin(@Nonnull GuildMemberJoinEvent event){
        ...
        Objects.requireNonNull(event.getGuild().getTextChannelById(862331304142045205L)).sendMessage(event.getMember().getNickname() + " ist gejoint").queue();
    ...
    public void onGuildMemberRemove(@Nonnull GuildMemberRemoveEvent event){
        Objects.requireNonNull(event.getGuild().getTextChannelById(862331304142045205L)).sendMessage(Objects.requireNonNull(event.getMember()).getEffectiveName() + " ist geleavt").queue();
        Objects.requireNonNull(event.getGuild().getVoiceChannelById(862716129608204328L)).getManager().setName(event.getGuild().getMemberCount() + " sitzen um's Feuer").queue();
        }
}
